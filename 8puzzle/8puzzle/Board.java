...
public class Board {
    ...
    public Board(int[][] blocks) {          // construct a board from an n-by-n array of blocks
        ...
        board_ = new int[dimension_*dimension_];
        for (int i = 0; i < dimension_; i++)
            for (int j = 0; j < dimension_; j++)
                board_[i*dimension_+j] = blocks[i][j];
    }
        ...
        for (int i = 0; i < dimension_; i++)
            for (int j = 0; j < dimension_; j++) 
                if (board_[i*dimension_+j] != 0) {
                    int value = board_[i*dimension_+j];
    ...
    public Board twin() {                   // a board that is obtained by exchanging any pair of blocks
        ...
        for (int i = 0; i < dimension_; i++)
            for (int j = 0; j < dimension_; j++) {
                if (board_[i*dimension_+j] != 0 && board_[i*dimension_+j] != i*dimension_+j+1) {
                    if (first == 0) {
                        first = i;
                        second = j;
                    }
                        ...
                        int temp = blocks[i][j];
                        blocks[i][j] = blocks[first][second];
                        blocks[first][second] = temp;
    ...
    public String toString() {              // string representation of this board (in the output format specified below)
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                s.append(String.format("%2d ", board_[i*dimension_+j]));
            }
            s.append("\n");
        }
        return s.toString();    
